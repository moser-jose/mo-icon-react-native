...
const COMPONENTS_EXPORTS_JS_FILE = path.resolve(ROOT_DIR, 'src', 'components-exports.js');
...
function writeComponentsExports(registryEntries: RegistryEntry[]): void {
  ...
  // JavaScript exports file
  const jsLines: string[] = [];
  jsLines.push("'use strict';");
  jsLines.push('/**');
  jsLines.push(' * Auto-generated file with named exports for all icon components');
  jsLines.push(' * This file is generated by scripts/generate-icons.ts');
  jsLines.push(' * Do not edit manually');
  jsLines.push(' */');
  jsLines.push('');

  sortedEntries.forEach((e) => {
    // Reuse the same relative path as registry.ts since both files are in src/
    jsLines.push(`exports.${e.componentName} = require('${e.relImportPath}').${e.componentName};`);
  });

  ensureDir(path.dirname(COMPONENTS_EXPORTS_JS_FILE));
  fs.writeFileSync(COMPONENTS_EXPORTS_JS_FILE, jsLines.join('\n'), 'utf8');
}
