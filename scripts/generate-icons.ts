...
const COMPONENTS_EXPORTS_FILE = path.resolve(ROOT_DIR, 'src', 'components-exports.ts');
const VARIANTS_DIR = path.resolve(ROOT_DIR, 'src', 'variants');
...
function listSvgFiles(dir: string): string[] {
  ...
  function walk(current: string) {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && entry.name.toLowerCase().endsWith('.svg')) results.push(full);
    }
  }
  walk(dir);
  return results;
...
function normalizeFilenames(rootDirectory: string): void {
  function walk(currentDirectory: string): void {
    ...
    for (const entry of entries) {
      ...
      console.log(`✔ Awaiting for icons to be generated...`);
      if (entry.isDirectory()) {
        const normalizedDirName = slugifySegment(entry.name);
        const newDirFullPath = path.join(currentDirectory, normalizedDirName);
        let dirToWalk = currentFullPath;

        if (normalizedDirName.length > 0 && currentFullPath !== newDirFullPath) {
          try {
            fs.renameSync(currentFullPath, newDirFullPath);
            dirToWalk = newDirFullPath;
          } catch (error) {
            console.error(`✖ Failed to rename dir ${currentFullPath} → ${newDirFullPath}`, error);
          }
        }
      ...
      if (entry.isFile()) {
        ...
        const slugBase = slugifySegment(baseName) || baseName.toLowerCase();
        const normalizedBase = slugBase;
        ...
        if (currentFullPath !== newFullPath) {
          try {
            ...
            console.error(`✖ Failed to rename ${currentFullPath} → ${newFullPath}`, error);
          }
        }
      }
    }
...
function prepareXml(svgXml: string, variant: string): string {
  ...
  let xml = optimized.replace(/\swidth="[^"]+"/gi, '').replace(/\sheight="[^"]+"/gi, '');
  ...
  if (svgOpenTagPattern.test(xml)) {
    if (viewBoxPattern.test(xml)) {
      ...
      xml = xml.replace(svgOpenTagPattern, (_match, attrs) => {
...
function makeComponentSource(componentName: string, xmlTemplate: string): string {
...
function writeRegistry(registryEntries: RegistryEntry[], variants: string[]): void {
  const lines: string[] = [];
  ...
  lines.push('');
  lines.push('');
  lines.push('export type IconType = ' + variants.map((v) => `'${v}'`).join(' | ') + ';');
  lines.push('');
  lines.push('export const iconRegistry: Record<IconType, Record<string, unknown>> = {');

  const byVariant = registryEntries.reduce<Record<string, RegistryEntry[]>>((acc, e) => {
...
function writeTypes(namesByVariant: Record<string, string[]>): void {
  ...
  lines.push('export type IconComponentProps = { size?: number; color?: string };');
  lines.push('');
  ensureDir(path.dirname(TYPES_FILE));
  fs.writeFileSync(TYPES_FILE, lines.join('\n'), 'utf8');
}

function writeComponentsExports(registryEntries: RegistryEntry[]): void {
  // TypeScript exports file
  const tsLines: string[] = [];
  tsLines.push('/* Author: @moser-jose */');
  tsLines.push('/* Date: ' + new Date().toISOString().split('T')[0] + ' */');
  tsLines.push('/*');
  tsLines.push('  - Auto-generated file with named exports for all icon components');
  tsLines.push('  - This file is generated by scripts/generate-icons.ts');
  tsLines.push('  - Do not edit manually');
  tsLines.push('*/');
  tsLines.push('');

  // Sort entries by component name for consistent output
  const sortedEntries = [...registryEntries].sort((a, b) =>
    a.componentName.localeCompare(b.componentName),
  );

  sortedEntries.forEach((e) => {
    // Reuse the same relative path as registry.ts since both files are in src/
    tsLines.push(`export { ${e.componentName} } from '${e.relImportPath}';`);
  });

  ensureDir(path.dirname(COMPONENTS_EXPORTS_FILE));
  fs.writeFileSync(COMPONENTS_EXPORTS_FILE, tsLines.join('\n'), 'utf8');
}

/**
 * Converts a variant name (e.g., "bold-duotone") to a category name (e.g., "BoldDuotone").
 */
function toCategoryName(variant: string): string {
  return variant
    .split(/[-_]/)
    .map((s) => s.charAt(0).toUpperCase() + s.slice(1))
    .join('');
...
/**
 * Removes the variant prefix from a component name.
 * Example: "BoldHome" with variant "bold" returns "Home".
 * If the resulting name starts with a number, prefixes it with "Icon" to make it a valid identifier.
 */
function removeVariantPrefix(componentName: string, variant: string): string {
  const variantPrefix = toCategoryName(variant);
  let exportName = componentName;

  if (componentName.startsWith(variantPrefix)) {
    exportName = componentName.slice(variantPrefix.length);
  }

  // If the name starts with a number, prefix it with "Icon" to make it a valid JavaScript identifier
...
/**
 * Groups registry entries by variant.
 */
function groupEntriesByVariant(registryEntries: RegistryEntry[]): Record<string, RegistryEntry[]> {
  return registryEntries.reduce<Record<string, RegistryEntry[]>>((acc, e) => {
    if (!acc[e.variant]) {
      acc[e.variant] = [];
    }
    acc[e.variant].push(e);
...
/**
 * Generates TypeScript export file content for a category.
 */
function generateCategoryTypeScriptExports(
  categoryName: string,
  ...
  lines.push('/* Date: ' + new Date().toISOString().split('T')[0] + ' */');
  lines.push('/*');
  lines.push(`  - Auto-generated file with exports for ${categoryName} category icons`);
  ...
  entries.forEach((e) => {
    const exportName = removeVariantPrefix(e.componentName, variant);
    // Adjust path to be relative to variants directory (../components/...)
    // TypeScript files should reference TypeScript source files (keep .tsx/.ts or no extension)
    const importPath = e.relImportPath.replace(/^\.\//, '../');
    lines.push(`export { ${e.componentName} as ${exportName} } from '${importPath}';`);
  });
...
/**
 * Generates category-specific export files (Bold.ts, Outline.ts, etc.)
 * that export components without their variant prefix.
 */
function writeCategoryExports(registryEntries: RegistryEntry[]): void {
  const byVariant = groupEntriesByVariant(registryEntries);
  const sortedVariants = Object.keys(byVariant).sort();

  ensureDir(VARIANTS_DIR);

  sortedVariants.forEach((variant) => {
    const categoryName = toCategoryName(variant);
    const entries = [...byVariant[variant]].sort((a, b) =>
      a.componentName.localeCompare(b.componentName),
    );

    const tsFile = path.resolve(VARIANTS_DIR, `${categoryName}.ts`);
    const tsContent = generateCategoryTypeScriptExports(categoryName, entries, variant);
...
function main(): void {
  ...
  for (const variant of variants) {
    ...
    for (const file of files) {
      ...
      const relImportPathWithExt =
        './' + path.relative(path.dirname(REGISTRY_FILE), outFile).replace(/\\\\/g, '/');
      const relImportPath = relImportPathWithExt.replace(/\.tsx$/i, '');
      registryEntries.push({ variant, nameKey, relImportPath, componentName });
    }
  }

  const namesByVariant = registryEntries.reduce<Record<string, string[]>>((acc, e) => {
    if (!acc[e.variant]) acc[e.variant] = [];
  ...
  writeCategoryExports(registryEntries);
